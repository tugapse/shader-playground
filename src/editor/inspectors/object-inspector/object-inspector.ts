...
export class ObjectInspector {
  ...
  getObjectType(newValue: Object): string {
    ...
    if (newValue instanceof Transform) result = 'transform';
    if (newValue instanceof GlEntity) result = 'entity';
    if (newValue instanceof EntityBehaviour) result = 'entityBehaviour';
    if (newValue instanceof Shader) result = 'shader';
    if (newValue instanceof Texture) result = 'texture';
    if (newValue instanceof NumberRange) result = 'range';
    if (
      newValue instanceof ColorMaterial ||
      newValue instanceof LitMaterial ||
      newValue instanceof UnlitMaterial
    )
      result = 'material';
    if (
      newValue instanceof Vector2 ||
      newValue instanceof Vector3 ||
      newValue instanceof Vector4 ||
      newValue instanceof Float32Array
    )
      result = 'vector234';
    if (newValue instanceof Boolean) result = 'boolean';
  ...
  protected isPropertyValid(key: string) {
    ...
    if (this.allowProperties.length > 0)
      return this.allowProperties.includes(key) && notPrivate;
    if (this.denyProperties.length > 0)
      return this.denyProperties.includes(key) == false && notPrivate;
    return notPrivate;
  }
  ...
  protected isValidPropertyType(key: string): boolean {
    ...
    const bool = this.showAllProperties
      ? true
  ...
  protected convertEnumToObject(something: any) {
