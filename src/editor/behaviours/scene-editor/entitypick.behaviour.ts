...
export class EntityPicker extends RendererBehaviour {
      ...
      if (this.canSelectEntity()) {
        ...
        const selectedEntity =
          this.selectedEntityId > 0
            ? (this.parent as Scene).objects[this.selectedEntityId - 1]
            : undefined;
  ...
  public renderAndGetSelected(): void {
    ...
    const posLoc = this._gl.getAttribLocation(
    ...
    for (const ob of obs) {
      ...
      if (
        renderer?.shader?._shaderProgram &&
    ...
    this._gl.readPixels(
      ...
      this.height - Mouse.mousePosition.y,
      1,
      1,
      this._gl.RGBA,
    ...
    if (this._pickingPixel[0] !== this.selectedEntityId) {
      ...
      const entity =
        this.selectedEntityId > 0 ? obs[this.selectedEntityId - 1] : null;
