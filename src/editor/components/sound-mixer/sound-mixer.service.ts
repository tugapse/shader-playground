...
export class SoundMixerPlaybackService {
  ...
  previewPreset(preset: SynthPreset) {
    ...
    if (preset.waveform === 'noise') {
      node = this.factory.triggerNoiseVoice(
        { ...preset, duration: preset.duration },
        now + 0.005,
      );
    } else {
      node = this.factory.triggerSynthVoice(
        preset.waveform as OscillatorType,
        { ...preset, duration: preset.duration },
        now + 0.05,
      );
    }
    ...
    this.clock.start((tick, schedulerTime) => {
      ...
      stepNotes.forEach((note) => {
        ...
        if (note.waveform === 'noise') {
          ...
          node = this.factory.triggerSynthVoice(
            note.waveform as OscillatorType,
            note,
            schedulerTime,
            prevFreq,
          );
    ...
    try {
      ...
      sortedSeq.forEach((note) => {
        ...
        if (note.waveform === 'noise') {
          ...
          offlineFactory.triggerSynthVoice(
            note.waveform as OscillatorType,
            note,
            note.startTime,
            prevFreq,
  ...
  loadDemoSong(
    ...
    const baseSequence: Omit<SynthNote, 'id'>[] = [];
    ...
    if (type === 'techno') {
      ...
      [1.0, 1.75, 3.25, 5.0, 5.75, 7.25].forEach((t, idx) => {
        baseSequence.push({
          ...pluck,
          startTime: t,
          duration: 0.4,
      ...
      [0.25, 0.75, 1.25, 2.25, 2.75, 3.25, 4.25, 5.25, 6.25, 6.75].forEach(
        (t) => {
          baseSequence.push({
            ...bass,
            startTime: t,
      ...
      [0.75, 1.25, 2.25, 3.5, 4.75, 5.25, 6.25, 7.5].forEach((t) => {
        baseSequence.push({
          ...bass,
          startTime: t,
          duration: 0.15,
      ...
      [0.0, 0.75, 2.0, 2.5, 4.0, 4.75, 6.0, 6.5].forEach((t) =>
        baseSequence.push({ ...kick, startTime: t, trackIndex: 0 }),
      );
      [0.5, 1.5, 2.25, 3.5, 4.5, 5.5, 6.25, 7.5].forEach((t) =>
        baseSequence.push({ ...snare, startTime: t, trackIndex: 0 }),
      );
    } else if (type === 'lofi') {
      ...
      [0.5, 1.25, 2.5, 3.0, 4.5, 5.25, 6.5, 7.0].forEach((t, i) => {
        baseSequence.push({
          ...pluck,
          startTime: t,
          duration: 0.3,
      ...
      [0.0, 1.2, 2.0, 3.2, 4.0, 5.2, 6.0, 7.2].forEach((t) =>
        baseSequence.push({ ...kick, startTime: t, trackIndex: 0 }),
      );
      [1.0, 3.0, 5.0, 7.0].forEach((t) =>
        baseSequence.push({
          ...snare,
          startTime: t,
          trackIndex: 0,
      ...
      [0.0, 0.75, 1.5, 2.0, 2.5, 3.25, 4.0, 4.75, 5.5, 6.0, 6.5, 7.25].forEach(
        (t) => {
          baseSequence.push({
            ...pluck,
            startTime: t,
      ...
      for (let i = 0; i < 16; i++) {
        if (i % 2 === 0)
          baseSequence.push({ ...kick, startTime: i * 0.5, trackIndex: 0 });
        if (i % 4 === 2)
          baseSequence.push({
            ...snare,
            startTime: i * 0.5,
    ...
    // Replicate the base 8-beat sequence to fill the requested target duration
    const PATTERN_LENGTH = 8.0;
    const fullSequence: SynthNote[] = [];
    const loopsRequired = Math.ceil(targetDuration / PATTERN_LENGTH);
    let idCounter = 0;

    for (let loop = 0; loop < loopsRequired; loop++) {
      const timeOffset = loop * PATTERN_LENGTH;

      for (const note of baseSequence) {
        const newStartTime = note.startTime! + timeOffset;

        if (newStartTime >= targetDuration) continue;
    ...
    const trackFrequencies = [880.0, 440.0, 220.0, 110.0, 55.0];
    const fineTunedSequence = fullSequence.map((note) => {
