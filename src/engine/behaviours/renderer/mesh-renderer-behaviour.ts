...
export class MeshRendererBehaviour extends RendererBehaviour {
  ...
  private readonly _spotPositionsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _spotDirectionsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _spotColorsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _spotInnerConeBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _spotOuterConeBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _spotConstAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _spotLinearAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _spotQuadraticAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );

  private readonly _dirDirectionsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _dirColorsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );

  private readonly _pointPositionsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _pointColorsBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS * 3,
  );
  private readonly _pointConstAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _pointLinearAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  private readonly _pointQuadraticAttBuf = new Float32Array(
    MeshRendererBehaviour.MAX_LIGHT_PASS,
  );
  ...
  override draw(): void {
    ...
    if (this.shader instanceof LitShader) {
      ...
      if (this.shadowMapTexture && this.shadowMapTexture.glTexture) {
        const lightEntity = this.parent.scene.lights.find(
          (obj) =>
        ...
        this.shader.setTexture(
          ShaderUniformsEnum.U_SHADOW_MAP,
          this.shadowMapTexture,
          2,
        );
        this.shader.setVec2(ShaderUniformsEnum.U_SHADOW_MAP_SIZE, [
        ...
        if (lightEntity) {
          let { lightMvpMatrix } = this.createLightMatrices(
          ...
          this.shader.setMat4(
            ShaderUniformsEnum.U_LIGHT_MVP_MATRIX,
            this._mvpScratch,
          );
          this.receiveShadows &&
    ...
    this.shader.setInt(
      ShaderUniformsEnum.U_FOG_ENABLED,
      this.fog.enabled ? 1 : 0,
    );
    if (this.fog.enabled) {
      this.shader.setVec3(
        ShaderUniformsEnum.U_FOG_COLOR,
        this.fog.color.toVec3(),
      );
      this.shader.setFloat(ShaderUniformsEnum.U_FOG_DENSITY, this.fog.density);
      this.shader.setFloat(
        ShaderUniformsEnum.U_FOG_DISTANCE,
        this.fog.distance,
      );
      this.shader.setInt(ShaderUniformsEnum.U_FOG_TYPE, this.fog.fogType);
      this.shader.setFloat(
        ShaderUniformsEnum.U_FOG_HEIGHT_FALLOFF,
        this.fog.heightFalloff,
      );
      this.shader.setFloat(
        ShaderUniformsEnum.U_FOG_BASE_HEIGHT,
        this.fog.baseHeight,
      );
    }
  ...
  protected override setShaderVariables(): void {
    this.shader!.setFloat(
      ...
      this.parent.scene.shadowmap.shadowstrength.value,
    );
  ...
  protected getNormalMapLocations(): void {
    if (this.shader?._shaderProgram) {
      this._normalMapUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        ShaderUniformsEnum.U_NORMAL_TEX,
      );
      this._specularMapUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        'u_specularMap',
      );
      this._roughnessMapUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        'u_roughnessMap',
      );
      this._aoMapUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        'u_aoMap',
      );
      this._emissiveMapUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        'u_emissiveMap',
      );

      this._worldMatrixUniformLocation = this._gl.getUniformLocation(
        this.shader._shaderProgram,
        ShaderUniformsEnum.U_WORLD_MATRIX,
      );
      this._worldInverseTransposeMatrixUniformLocation =
        this._gl.getUniformLocation(
          this.shader._shaderProgram,
          ShaderUniformsEnum.U_WORLD_INVERSE_TRANSPOSE_MATRIX,
        );
      this._tangentAttributeLocation = this._gl.getAttribLocation(
        this.shader._shaderProgram,
        ShaderUniformsEnum.A_TANGENT,
      );
      this._bitangentAttributeLocation = this._gl.getAttribLocation(
        this.shader._shaderProgram,
  ...
  protected bindMap(
    ...
    unit: number,
    fallbackType: 'white' | 'black' | 'normal',
  ) {
    if (!location) return;
  ...
  protected setNormalMapsInformation(): void {
    ...
    this.bindMap(
      material.normalTex,
      this._normalMapUniformLocation,
      1,
      'normal',
    );
    this.bindMap(
      material.specularTex,
      this._specularMapUniformLocation,
      3,
      'white',
    );
    this.bindMap(
      material.roughnessTex,
      this._roughnessMapUniformLocation,
      4,
      'white',
    );
    ...
    this.bindMap(
      ...
      this._emissiveMapUniformLocation,
      6,
      'black',
    );
  }
  ...
  protected setLightInformation(): void {
    if (this.shader instanceof LitShader) {
      const lights = this.parent.scene.lights.filter(
        (light) => light.active && light.show,
      );
      const ambientLight = lights.find(
        (l) => l.entityType === EntityType.LIGHT_AMBIENT,
      );

      if (ambientLight) {
        this.shader.setVec4(
          ShaderUniformsEnum.U_AMBIENT_LIGHT,
          ambientLight.color.toVec4(),
        );
      } else {
        this.shader.setVec4(
          ShaderUniformsEnum.U_AMBIENT_LIGHT,
  ...
  protected createLightObjectInfo(sceneLights: Light[]): void {
    ...
    const pointLights: PointLight[] = sceneLights.filter(
      (e) => e.entityType === EntityType.LIGHT_POINT,
    ) as PointLight[];
    const spotLights: SpotLight[] = sceneLights.filter(
      (e) => e.entityType === EntityType.LIGHT_SPOT,
    ) as SpotLight[];
  ...
  protected loadSpotLights(spotLights: SpotLight[]): void {
    ...
    const uNumSpotLightsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_numSpotLights',
    );
    const uSpotPositionsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightPositions[0]',
    );
    const uSpotDirectionsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightDirections[0]',
    );
    const uSpotColorsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightColors[0]',
    );
    const uSpotInnerConeCosLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightInnerConeCos[0]',
    );
    const uSpotOuterConeCosLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightOuterConeCos[0]',
    );
    const uSpotConstAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightConstantAtts[0]',
    );
    const uSpotLinearAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightLinearAtts[0]',
    );
    const uSpotQuadraticAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_spotLightQuadraticAtts[0]',
    );
    ...
    if (count > 0) {
      if (uSpotPositionsLoc)
        this._gl.uniform3fv(
          uSpotPositionsLoc,
          this._spotPositionsBuf.subarray(0, count * 3),
        );
      if (uSpotDirectionsLoc)
        this._gl.uniform3fv(
          uSpotDirectionsLoc,
          this._spotDirectionsBuf.subarray(0, count * 3),
        );
      if (uSpotColorsLoc)
        this._gl.uniform3fv(
          uSpotColorsLoc,
          this._spotColorsBuf.subarray(0, count * 3),
        );
      if (uSpotInnerConeCosLoc)
        this._gl.uniform1fv(
          uSpotInnerConeCosLoc,
          this._spotInnerConeBuf.subarray(0, count),
        );
      if (uSpotOuterConeCosLoc)
        this._gl.uniform1fv(
          uSpotOuterConeCosLoc,
          this._spotOuterConeBuf.subarray(0, count),
        );
      if (uSpotConstAttsLoc)
        this._gl.uniform1fv(
          uSpotConstAttsLoc,
          this._spotConstAttBuf.subarray(0, count),
        );
      if (uSpotLinearAttsLoc)
        this._gl.uniform1fv(
          uSpotLinearAttsLoc,
          this._spotLinearAttBuf.subarray(0, count),
        );
      if (uSpotQuadraticAttsLoc)
        this._gl.uniform1fv(
          uSpotQuadraticAttsLoc,
          this._spotQuadraticAttBuf.subarray(0, count),
        );
    }
  }
  ...
  protected loadDirectionalLights(directionalLights: DirectionalLight[]): void {
    ...
    const uNumDirLightsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_numDirectionalLights',
    );
    const uDirDirectionsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_directionalLightDirections[0]',
    );
    const uDirColorsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_directionalLightColors[0]',
    );
    ...
    if (count > 0) {
      if (uDirDirectionsLoc)
        this._gl.uniform3fv(
          uDirDirectionsLoc,
          this._dirDirectionsBuf.subarray(0, count * 3),
        );
      if (uDirColorsLoc)
        this._gl.uniform3fv(
          uDirColorsLoc,
          this._dirColorsBuf.subarray(0, count * 3),
        );
    }
  }
  ...
  protected loadPointLights(pointLights: PointLight[]): void {
    ...
    const uNumPointLightsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_numPointLights',
    );
    const uPointPositionsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_pointLightPositions[0]',
    );
    const uPointColorsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_pointLightColors[0]',
    );
    const uPointConstAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_pointLightConstantAtts[0]',
    );
    const uPointLinearAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_pointLightLinearAtts[0]',
    );
    const uPointQuadraticAttsLoc = this._gl.getUniformLocation(
      this.shader._shaderProgram!,
      'u_pointLightQuadraticAtts[0]',
    );
    ...
    if (count > 0) {
      if (uPointPositionsLoc)
        this._gl.uniform3fv(
          uPointPositionsLoc,
          this._pointPositionsBuf.subarray(0, count * 3),
        );
      if (uPointColorsLoc)
        this._gl.uniform3fv(
          uPointColorsLoc,
          this._pointColorsBuf.subarray(0, count * 3),
        );
      if (uPointConstAttsLoc)
        this._gl.uniform1fv(
          uPointConstAttsLoc,
          this._pointConstAttBuf.subarray(0, count),
        );
      if (uPointLinearAttsLoc)
        this._gl.uniform1fv(
          uPointLinearAttsLoc,
          this._pointLinearAttBuf.subarray(0, count),
        );
      if (uPointQuadraticAttsLoc)
        this._gl.uniform1fv(
          uPointQuadraticAttsLoc,
          this._pointQuadraticAttBuf.subarray(0, count),
        );
    }
  }
}
